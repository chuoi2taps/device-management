...
type RestOf<T extends IDevice | NewDevice> = Omit<T, 'purchaseDate'> & {
...
export class DeviceService {
  ...
  protected convertDateFromClient<T extends IDevice | NewDevice | PartialUpdateDevice>(device: T): RestOf<T> {
    return {
      ...device,
      purchaseDate: device.purchaseDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDevice: RestDevice): IDevice {
    return {
      ...restDevice,
      purchaseDate: restDevice.purchaseDate ? dayjs(restDevice.purchaseDate) : undefined,
    };
  }
